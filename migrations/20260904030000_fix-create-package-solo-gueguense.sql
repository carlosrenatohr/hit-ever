-- ============================================================================
-- Fix create_package: add solo-guegue → global_connection mapping
-- ============================================================================
-- The create_package RPC (20260815030000) maps agency → default provider.
-- Solo Guegue uses Global Connection as its sole provider.
-- Additive: widens the CASE mapping; no existing rows affected.
-- ============================================================================
  ...
  if p_provider_code is not null then
    ...
    where active
      and (case v_agency
             ...
             when 'solo-guegue'  then code = 'global_connection'
             else false
           end)
    limit 1;
    if v_provider is null then
